...
const TAB_OPTIONS = ["All Posts", "Recent", "Popular", "Reported"];

const baseTabClass = "px-5 py-2.5 rounded-xl text-sm font-medium transition-all";
const baseActionBtn = "flex items-center gap-2 bg-[#1A7A4A] text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#15633C] whitespace-nowrap";
...
const ActionMenu = memo(function ActionMenu({ post, onView }) {
  const [open, setOpen] = useState(false);
  const handleToggle = useCallback(() => setOpen((prev) => !prev), []);
  const handleView = useCallback(() => { onView(post); setOpen(false); }, [onView, post]);
  const handleClose = useCallback(() => setOpen(false), []);

  return (
    <div className="relative">
      <button onClick={handleToggle} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700" aria-label="Open action menu">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-20">
            <button onClick={handleView} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-600 hover:bg-gray-50" aria-label="View post">
              <Eye size={14} /> View Post
            </button>
            <button className="flex items-center gap-2 w-full px-3 py-2 text-sm text-red-500 hover:bg-red-50" aria-label="Delete post">
...
const DetailModal = memo(function DetailModal({ post, onClose }) {
  if (!post) return null;
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="px-6 pb-6 flex gap-2">
          <button className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50" onClick={onClose} aria-label="Close modal">Close</button>
        </div>
      </div>
    </div>
  );
...
export default function CommunityPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(`${BASE_URL}/api/v1/community/home`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .catch((e) => {
        console.error(e);
        window.alert("Failed to load community posts.");
        setError(true);
      })
  ...
  const handleSelectPost = useCallback((post) => setSelected(post), []);
  const handleCloseModal = useCallback(() => setSelected(null), []);

  const filteredPosts = posts.filter(p => {
    const matchSearch = search === "" || (p.title || p.content || "").toLowerCase().includes(search.toLowerCase()) || (p.userName || "").toLowerCase().includes(search.toLowerCase());
    if (activeTab === "All Posts") return matchSearch;
    if (activeTab === "Reported") return matchSearch && (p.status === "Reported");
    return matchSearch;
  });

  const totalMembers = posts.length * 18 || 12450;
    ...
    <>
      ...
      <div className="space-y-6">
          ...
          {[
            ...
            <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-9 h-9 rounded-xl bg-[#E8F5EE] flex items-center justify-center"><stat.icon size={18} className="text-[#1A7A4A]" /></div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {loading ? <Spinner /> : error ? <EmptyState message="Could not load community posts." /> : filteredPosts.length === 0 ? <EmptyState message="No posts found matching your criteria." /> : (
                  ...
                  {filteredPosts.map((post, i) => (
                    <tr key={post.id || i} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-[#E8F5EE] flex items-center justify-center text-[#1A7A4A] font-bold text-xs">{(post.userName || "User").slice(0, 2).toUpperCase()}</div>
                          <span className="font-medium text-gray-800">{post.userName || "Anonymous"}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4 max-w-xs"><p className="line-clamp-2 text-gray-700 text-xs leading-relaxed">{post.title || post.content || post.preview || "Community discussion"}</p></td>
                      <td className="px-4 py-4 hidden md:table-cell"><span className="inline-block px-3 py-1 text-xs rounded-full bg-purple-50 text-purple-600">{post.category || "General"}</span></td>
                      <td className="px-4 py-4 text-gray-500 text-xs hidden sm:table-cell">{post.time || "2 hours ago"}</td>
                      ...
                      <td className="px-4 py-4"><StatusBadge status={post.status || "Published"} /></td>
                      <td className="px-4 py-4"><ActionMenu post={post} onView={handleSelectPost} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
