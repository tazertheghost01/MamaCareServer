...
export default function RemindersScreen() {
  ...
  const allReminders = [
    ...appointments.map((a) => ({
      id: a.id, type: "appointment", icon: "calendar-outline",
      title: a.title,
      date: a.appointmentDate ? new Date(a.appointmentDate).toLocaleDateString("en-NG", {
        month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit",
      }) : "",
      sub: a.daysUntilAppointment != null ? `In ${a.daysUntilAppointment} days` : "",
      completed: a.status === "COMPLETED", color: "#F0FAF4",
    })),
    ...medications.map((m) => ({
      id: m.medicationId || m.id, type: "medication", icon: "medical-outline",
      title: m.medicationName, date: m.reminderTime?.slice(0, 5) || "",
      sub: "Daily reminder", completed: m.isTaken || false, color: "#FFF8F0",
    })),
    {
      id: "walk", type: "walk", icon: "walk-outline",
      title: "Evening Walk", date: "Today 6:00 PM",
      sub: "Lets move gently", completed: false, color: "#F0F4FF",
    },
  ];
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F8F8F8" }}>
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "flex-end" }}>
          <View style={{ backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, maxHeight: "90%" }}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {addType === "appointment" ? (
                <View style={{ gap: 12 }}>
                  <FormField placeholder="Title" value={apptForm.title} onChangeText={(v) => setApptForm({ ...apptForm, title: v })} />
                ...
                <View style={{ gap: 12 }}>
                  <FormField placeholder="Medication name" value={medForm.medicationName} onChangeText={(v) => setMedForm({ ...medForm, medicationName: v })} />
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <View style={{
          ...
          <Switch value={alertsEnabled} onValueChange={setAlertsEnabled} trackColor={{ false: "#E0E0E0", true: "#2D7A4F" }} thumbColor="#fff" />
        </View>
            ...
            filtered.map((r: any, i) => (
              <TouchableOpacity key={i} style={{
                ...
                {r.type === "medication" && !r.completed && (
                  <TouchableOpacity onPress={() => handleMarkMedTaken(r.id)} style={{ backgroundColor: "#E8F5EE", borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 }}>
                    <Text style={{ fontSize: 11, color: "#2D7A4F", fontWeight: "700" }}>Taken</Text>
                  </TouchableOpacity>
                )}
                {r.completed && <Ionicons name="checkmark-circle" size={22} color="#2D7A4F" />}
                {!r.completed && r.type !== "medication" && <Ionicons name="ellipsis-horizontal" size={20} color="#CCC" />}
              </TouchableOpacity>
            ))
          )}
        </View>
